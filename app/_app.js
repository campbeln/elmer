...
module.exports = async function ($app, $express, $httpServer) {
    $app.app = {
        ...
        process: (function () {
            //#
            function removeUuid() {
                let bReturnVal = $app.type.uuid.is(vTargets) && $app.type.is.value($app.data.processes[sUuid]);

                //#
                if (bReturnVal) {
                    delete $app.data.processes[sUuid];
                }
            ...
            return {
                //#
                start: function (oRequest) {
                    let sUuid = $app.type.uuid();

                    //#
                    $app.data.processes[sUuid] = {
                        //uuid: sUuid,
                        complete: false,
                        time: {
                            start: Date.now(),
                        ...
                        request: $app.type.obj.clone(
                            oRequest, //# https://expressjs.com/en/api.html#req.ips
                            ["baseUrl", "body", "cookies", "method", "path", "protocol", "query", "route", "secure", "signedCookies"]
                        ),
                        response: {},
                        progress: []
                    };
                ...
                //#
                purge: function (vTargets) {
                    let i,
                        dBefore = $app.type.date.mk(vTargets)
                        a_sReturnVal = []
                    ;

                    //#
                    if ($app.type.str.is(vTargets, true)) {
                        if (removeUuid(vTargets)) {
                            a_sReturnVal.push(vTargets);
                        }
                    }
                    //#
                    else if ($app.type.arr.is(vTargets)) {
                        for (i = 0; i < vTargets.length; i++) {
                            if (removeUuid(vTargets[i])) {
                                a_sReturnVal.push(vTargets[i]);
                            }
                        }
                    }
                    //#
                    else {
                        let a_sKeys = $app.type.obj.ownKeys($app.data.processes);

                        for (i = 0; i < a_sKeys.length; i++) {
                            if ($app.type.date.cmp($app.resolve($app.data.processes, [a_sKeys[i], "time", "stop"]), dBefore) === -1) {
                                delete $app.data.processes[a_sKeys[i]];
                                a_sReturnVal.push(a_sKeys[i]);
                            }
                        }
                    }
